...
impl Server {
    ...
    pub async fn listen_and_serve(&self) -> Result<()> {
        info!("[server] bind on address '{}'", self.addr);
        let bind: SocketAddr = self.addr.parse().context("parse listen address")?;
        let listener = TcpListener::bind(&bind)
            .await
            .context("bind tcp listener")?;

        info!("[server] begin to accept connection");
        loop {
...
mod tests {
    ...
    async fn test_server() {
        ...
        let server = Server::new(addr, password);
